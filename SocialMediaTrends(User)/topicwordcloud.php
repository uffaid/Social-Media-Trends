...
function word_topicfrequency($array_clean){
$freq_list=array();
...
function freq_topicfilter($array,$filter){
...
$query=("SELECT * FROM twitterrawdata ");
...
while($row = mysql_fetch_array($result))
   {
	 ...
	 $array_clean=cleanKeywordsTopic($array_words);//Calling function cleanKeywordsTopic to get clean data free of stop words
	 $raw_array=word_topicfrequency($array_clean);//Calling function word_topicfrequency to get word frequency
	$pure_array=freq_topicfilter($raw_array,10);//Calling function freq_topicfilter to filter words less than 10
?>
...
<script>	
  ...
  function draw(words) {
    d3.select("#index_topicwordcloud").append("svg")
        .attr("width", 600)
        .attr("height", 300)
