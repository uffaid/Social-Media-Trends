...
if(isset($_REQUEST['topic_id'])){
	$selected_topic=$_REQUEST['topic_id'];
}
$query="SELECT * FROM twitterrawdata where topic_id=".$selected_topic;
//echo $query;
...
while($row = mysql_fetch_array($result))
   {
	...
	$pure_array=freq_filter($raw_array,25);
	// print_r ($raw_array);
	//print_r($pure_array);
?>
...
?>
<div id="keyword_wordCloud" style="position:absolute; top:40px; left:33%; width=49%; border:2px solid; border-radius:10px; box-shadow: 5px 5px 5px #7A0000"></div>
<script>	
  ...
  function draw(words) {
    d3.select("#keyword_wordCloud").append("svg")
        .attr("width", 700)
        .attr("height", 300)
